...
def get_tx_issue_hex(node, asset_name, asset_quantity, asset_units=0):
    ...
    outputs = {
        ...
        change_address: truncate(float(unspent['amount']) - 500.01),
        to_address: {
            'issue': {
...
class RawAssetTransactionsTest(RavenTestFramework):
    ...
    def reissue_tampering_test(self):
        ...
        outputs = {
            ...
            n0.getnewaddress(): truncate(float(unspent['amount']) - 100.01),
            to_address: {
                'reissue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1000,
    ...
    def issue_reissue_transfer_test(self):
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 100,
            change_address: truncate(float(unspent['amount']) - 100.01),
            to_address: {
                'reissue': {
                    'asset_name':       'TEST_ASSET',
                    'asset_quantity':   1000,
