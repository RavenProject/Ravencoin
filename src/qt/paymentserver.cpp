...
const int YOTTAFLUX_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString YOTTAFLUX_IPC_PREFIX("yottaflux:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(YOTTAFLUX_IPC_PREFIX, Qt::CaseInsensitive)) // yottaflux: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(YOTTAFLUX_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(YOTTAFLUX_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(YOTTAFLUX_IPC_PREFIX, Qt::CaseInsensitive)) // yottaflux: URI
    {
