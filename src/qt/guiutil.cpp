...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Yottaflux address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new YottafluxAddressEntryValidator(parent));
    widget->setCheckValidator(new YottafluxAddressCheckValidator(parent));
}
...
bool parseYottafluxURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no yottaflux: URI
    if(!uri.isValid() || uri.scheme() != QString("yottaflux"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!YottafluxUnits::parse(YottafluxUnits::YAI, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseYottafluxURI(QString uri, SendCoinsRecipient *out)
{
    // Convert yottaflux:// to yottaflux:
    //
    //    Cannot handle this later, because yottaflux:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("yottaflux://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "yottaflux:");
    }
    QUrl uriInstance(uri);
    return parseYottafluxURI(uriInstance, out);
}

QString formatYottafluxURI(const SendCoinsRecipient &info)
{
    QString ret = QString("yottaflux:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(YottafluxUnits::format(YottafluxUnits::YAI, info.amount, false, YottafluxUnits::separatorNever));
        paramCount++;
    }
...
bool openYottafluxConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(YOTTAFLUX_CONF_FILENAME);
    ...
    /* Open yottaflux.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Yottaflux.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Yottaflux (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Yottaflux (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Yottaflux*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "yottaflux.desktop";
    return GetAutostartDir() / strprintf("yottaflux-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a yottaflux.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Yottaflux\n";
        else
            optionFile << strprintf("Name=Yottaflux (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the yottaflux app
...
bool GetStartOnSystemStartup()
{
    CFURLRef yottafluxAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (yottafluxAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, yottafluxAppUrl);

    CFRelease(yottafluxAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add yottaflux app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, yottafluxAppUrl, nullptr, nullptr);
    }
