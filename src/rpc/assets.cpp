...
UniValue UpdateAddressQualifier(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    // Always transfer 1 of the qualifier tokens to the change address generated by this wallet
    vTransfers.emplace_back(std::make_pair(CAssetTransfer(tag_name, 1 * COIN), EncodeDestination(keyID)));
...
UniValue addqualifiertoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
                "addqualifiertoaddress tag_name address\n"
                ...
                "3. \"to_address\"          (string), required) the address that will be assigned the tag\n"
...
UniValue removequalifierfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
                "removequalifierfromaddress tag_name address\n"
                + RestrictedActivationWarning() +
                "\nRemove a tag from a address\n"

                "\nArguments:\n"
                "1. \"tag_name\"            (string, required) the name of the tag you are removing from the address\n"
                "3. \"to_address\"          (string), required) the address that the tag will be removed from\n"
...
{ //  category    name                          actor (function)             argNames
  //  ----------- ------------------------      -----------------------      ----------
    ...
    { "assets",   "addqualifiertoaddress",      &addqualifiertoaddress,      {"tag_name", "to_address"}},
    { "assets",   "removequalifierfromaddress", &removequalifierfromaddress, {"tag_name", "to_address"}}
};
