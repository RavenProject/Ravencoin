...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** YAI START */
    case OP_YAI_ASSET              : return "OP_YAI_ASSET";
    /** YAI END */
    ...
    if (this->size() > 31) {
        if ((*this)[25] == OP_YAI_ASSET) { // OP_YAI_ASSET is always in the 25 index of the script if it exists
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == YAI_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == YAI_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == YAI_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == YAI_R) {
...
{
    // 1 OP_YAI_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_YAI_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_YAI_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_YAI_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_YAI_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
