...
static const char QULAIFIER_ADDRESS_FLAG = 'T';
...
{
    int8_t i = 1;
    return Write(std::make_pair(QULAIFIER_ADDRESS_FLAG, std::make_pair(address, tag)), i);
}

bool CRestrictedDB::ReadAddressQualifier(const std::string &address, const std::string &tag)
{
    int8_t i;
    return Read(std::make_pair(QULAIFIER_ADDRESS_FLAG, std::make_pair(address, tag)), i);
}

bool CRestrictedDB::EraseAddressQualifier(const std::string &address, const std::string &tag)
{
    return Erase(std::make_pair(QULAIFIER_ADDRESS_FLAG, std::make_pair(address, tag)));
}
...
bool CRestrictedDB::GetAddressQualifiers(std::string& address, std::vector<std::string>& qualifiers)
{
    ...
    pcursor->Seek(std::make_pair(QULAIFIER_ADDRESS_FLAG, std::make_pair(address, std::string())));
