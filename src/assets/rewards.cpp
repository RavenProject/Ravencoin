...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for YAI
    ...
    if (p_rewardSnapshot.strDistributionAsset != "YAI") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is YAI with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using YAI differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "YAI") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Yottaflux address (already validated during ownership snapshot creation)
