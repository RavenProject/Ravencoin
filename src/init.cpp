 ...
 * between chickadeed, and chickadee-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("chickadee-shutoff");
    ...
    FlushWallets();
#endif
    GenerateChickadees(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), CHICKADEE_CONF_FILENAME));
    if (mode == HMM_CHICKADEED)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CHICKADEE_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/lukepighetti/x16rc>";
    const std::string URL_WEBSITE = "<https://x16rc.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("chickadee-loadblk");
...
/** Sanity checks
 *  Ensure that Chickadee is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Chickadee process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", CHICKADEE_CONF_FILENAME)).string());
    ...
    GenerateChickadees(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
