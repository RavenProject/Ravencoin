...
The RPM spec file provided here is for Yottaflux-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://yottaflux.ai/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Yottaflux git repository](https://github.com/yottaflux/Yottaflux) but are not part of
...
Currently only `Source10` is used, for the example `yottaflux.conf` file.
...
in the yottaflux git repository. At present only an SVG version of the Yottaflux
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the yottaflux
...
needed when 0.12.1 is released, a proper fix is already in the Yottaflux git
master branch.
...
idea to build Yottaflux Core against a static version of that library compiled
...
The `yottaflux.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `yottaflux-server` package. At this point, most RPM based
...
The `yottaflux-server` package should have SELinux support. How to properly do
...
Yottaflux-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/yottaflux/). He has been
packaging Yottaflux for Fedora at least since 2012.
...
4. I renamed the `yottaflux` package that contains the Qt GUI to `yottaflux-core` as
that appears to be how the general population refers to it, in contrast to
`yottaflux-xt` or `yottaflux-classic`. I wanted to make sure the general population
