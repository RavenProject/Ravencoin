...
The RPM spec file provided here is for Chickadee-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://x16rc.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Chickadee git repository](https://github.com/lukepighetti/x16rc) but are not part of
...
Currently only `Source10` is used, for the example `chickadee.conf` file.
...
in the chickadee git repository. At present only an SVG version of the Chickadee
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the chickadee
...
needed when 0.12.1 is released, a proper fix is already in the Chickadee git
master branch.
...
idea to build Chickadee Core against a static version of that library compiled
...
The `chickadee.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `chickadee-server` package. At this point, most RPM based
...
The `chickadee-server` package should have SELinux support. How to properly do
...
Chickadee-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/chickadee/). He has been
packaging Chickadee for Fedora at least since 2012.
...
4. I renamed the `chickadee` package that contains the Qt GUI to `chickadee-core` as
that appears to be how the general population refers to it, in contrast to
`chickadee-xt` or `chickadee-classic`. I wanted to make sure the general population
