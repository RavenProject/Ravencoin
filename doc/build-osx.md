...
## Build Yottaflux Core

1. Clone the Yottaflux Core source code:
    ```shell
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Yottaflux Core may be
...
## Running
Yottaflux Core is now available at `./src/yottafluxd`
