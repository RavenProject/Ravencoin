...
This guide describes how to build yottafluxd and command-line utilities on OpenBSD.
...
Do not use `pkg_add boost`! The boost version installed thus is compiled using the `g++` compiler not `eg++`, which will result in a conflict between `/usr/local/lib/libestdc++.so.XX.0` and `/usr/lib/libstdc++.so.XX.0`, resulting in a test crash:
    ...
    test_yottaflux:/usr/lib/libstdc++.so.57.0: /usr/local/lib/libestdc++.so.17.0 : WARNING: symbol(_ZN11__gnu_debug17_S_debug_me ssagesE) size mismatch, relink your program
    ...
...
YOTTAFLUX_ROOT=$(pwd)
BOOST_PREFIX="${YOTTAFLUX_ROOT}/boost"
...
# Build w/ minimum configuration necessary for yottaflux
...
```bash
# Pick some path to install BDB to, here we create a directory within the yottaflux directory
YOTTAFLUX_ROOT=$(pwd)
BDB_PREFIX="${YOTTAFLUX_ROOT}/db4"
mkdir -p $BDB_PREFIX
...
with many 'local symbol discarded' errors:
    ...
    local symbol 150: discarded in section `.text._ZN10tinyformat6detail14FormatIterator6finishEv' from libyottaflux_util.a(libyottaflux_util_a-random.o)
    local symbol 151: discarded in section `.text._ZN10tinyformat6detail14FormatIterator21streamStateFromFormatERSoRjPKcii' from libyottaflux_util.a(libyottaflux_util_a-random.o)
    local symbol 152: discarded in section `.text._ZN10tinyformat6detail12convertToIntIA13_cLb0EE6invokeERA13_Kc' from libyottaflux_util.a(libyottaflux_util_a-random.o)
